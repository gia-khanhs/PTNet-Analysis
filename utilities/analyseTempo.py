...
def multisrcBfs01(): #special case of dijkstra & bfs: 0/1 bfs
    global dNodes, dEdges, dStations
    dNodes, dEdges = saveNLoadAnalysingGraph(revGraphInTime(14407, 14407 + 5 * 60, True))
    print(dNodes)
    print(dEdges)

    optimised = set()

    sources = list(range(1, len(dNodes) + 1))
    sources.reverse()

    for source in sources:
        if source in optimised: continue;

        transfer = [INT_MAX] * (len(dNodes) + 1)
        transfer[source] = 0
        trace = [-1] * (len(dNodes) + 1)
    ...
    return (transfer, trace)

def revGraphInTime(tDesiredDep, tEnd = None, mimicPaper = False):
    ensureGraphBuilt(mimicPaper)
    global stations, nodes, nodesById, edges, nStation, nNode, isMimic
    ...
    root = 0 #created to run shortest path only once from the root (this node has an edge to every arrival node)
    ...
    for u in range(l, r + 1):
        for v, w in edges[u]:
            ...
            # if not newEdges.get(u - l + 1): newEdges[u - l + 1] = []
            # newEdges[u - l + 1].append((v - l + 1, w))
            if not newEdges.get(v - l + 1): newEdges[v - l + 1] = []
            newEdges[v - l + 1].append((u - l + 1, w))

    print("Built reversed graph in time window for analysation!")
